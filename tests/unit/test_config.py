...
def test_macro_policy_ingest_flags_default_off():
    ...
    assert settings.macro_market_shadow_ingest_enabled is False
...
def test_macro_policy_ingest_flags_read_env(tmp_path, monkeypatch):
    for key in (
        ...
        "UW_SCAN_MACRO_MARKET_SHADOW_INGEST_ENABLED",
    ):
    ...
    env_path.write_text(
        "\n".join(
            [
                ...
                "UW_SCAN_MACRO_MARKET_SHADOW_INGEST_ENABLED=on",
            ]
        )
    )
    ...
    assert settings.macro_market_shadow_ingest_enabled is True

# docs/research/regime — CRI methodology research

## Files

- `cri-methodology.md` — **source of truth** for CRI math, calibration, and design decisions. Read this before changing any threshold in `src/uw_scan/cards/cri_scoring.py`.
- `cri-validation.ipynb` — out-of-sample walk-forward validation against 20 years of CBOE vol-complex data. Section 9 is the honest accuracy breakdown.
- `cri-backtest.{md,csv}` — generated by `scripts/backtest_cri.py`. Recent-history (trailing months bounded by SPY daily_ohlc) sanity check. Regenerate after any calibration change and review the diff.

## When to update

- After changing any constant in `cri_scoring.py`: update §3 of `cri-methodology.md` with the new threshold and rationale.
- After running the backtest: commit the regenerated `cri-backtest.{md,csv}` so reviewers can see the impact.

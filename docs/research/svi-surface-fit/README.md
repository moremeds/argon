...
- Overlay PNGs (`figs/*.png`) are regenerated by the reproduce command; they are **not**
  committed (repo `.gitignore` blocks `*.png`). `overlays.csv` is the lossless durable trace.

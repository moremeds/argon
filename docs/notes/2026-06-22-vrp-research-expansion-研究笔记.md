...
## 2. ⚠️ 这是 markout 信号研究，**不是**可交易回测（entry / exit / P&L 在哪？）
...
> 缺的是**期权结构定价 + 对冲模型**（把 vol points 翻译成定义风险结构的美元盈亏）。这是下面 §8 列的下一步。
...
## 4. 实测结果（2026-06-22 本地预跑，已做 corp-action 调整）
...
## 8. 下一步（从 markout 到可交易回测）

研究层已证明 edge 在。真正缺的、也是最有价值的下一步是把它**翻译成一个定义风险的、带 P&L 的策略回测**：

1. **定义风险的空波动率结构回测**（headline）：按 §2 的 7 点路线图建一个 `vrp_strategy_backtest` —— 在 RICH 信号入场，

...
| dark_pool_events | freshness_only | none | none |  | equity_session | UW serves this endpoint for past dates (probed 2026-08-16, HTTP 200 with rows). Blocked only by missing date plumbing in pipeline.run_single_stock — full_scan_once takes no `date`. Not a provider refusal. | 2026-08-16 |
| exposures_by_expiry_strike | freshness_only | none | none |  | equity_session | UW serves this endpoint for past dates (probed 2026-08-16, HTTP 200 with rows). Blocked only by missing date plumbing in pipeline.run_single_stock — full_scan_once takes no `date`. Not a provider refusal. | 2026-08-16 |
| exposures_summary | freshness_only | none | none |  | equity_session | UW serves this endpoint for past dates (probed 2026-08-16, HTTP 200 with rows). Blocked only by missing date plumbing in pipeline.run_single_stock — full_scan_once takes no `date`. Not a provider refusal. | 2026-08-16 |
...
| greeks_by_expiry_strike | freshness_only | none | none |  | equity_session | UW serves this endpoint for past dates (probed 2026-08-16, HTTP 200 with rows). Blocked only by missing date plumbing in pipeline.run_single_stock — full_scan_once takes no `date`. Not a provider refusal. | 2026-08-16 |
...
| interpolated_iv_snapshots | freshness_only | none | none |  | equity_session | UW serves this endpoint for past dates (probed 2026-08-16, HTTP 200 with rows). Blocked only by missing date plumbing in pipeline.run_single_stock — full_scan_once takes no `date`. Not a provider refusal. | 2026-08-16 |
| iv_rank_history | freshness_only | none | none |  | equity_session | UW serves this endpoint for past dates (probed 2026-08-16, HTTP 200 with rows). Blocked only by missing date plumbing in pipeline.run_single_stock — full_scan_once takes no `date`. Not a provider refusal. | 2026-08-16 |
...
| iv_term_snapshots | freshness_only | none | none |  | equity_session | UW serves this endpoint for past dates (probed 2026-08-16, HTTP 200 with rows). Blocked only by missing date plumbing in pipeline.run_single_stock — full_scan_once takes no `date`. Not a provider refusal. | 2026-08-16 |
...
| max_pain_by_expiry | freshness_only | none | none |  | equity_session | UW serves this endpoint for past dates (probed 2026-08-16, HTTP 200 with rows). Blocked only by missing date plumbing in pipeline.run_single_stock — full_scan_once takes no `date`. Not a provider refusal. | 2026-08-16 |
...
| oi_by_strike | freshness_only | none | none |  | equity_session | UW serves this endpoint for past dates (probed 2026-08-16, HTTP 200 with rows). Blocked only by missing date plumbing in pipeline.run_single_stock — full_scan_once takes no `date`. Not a provider refusal. | 2026-08-16 |
| oi_change_events | freshness_only | none | none |  | equity_session | UW serves this endpoint for past dates (probed 2026-08-16, HTTP 200 with rows). Blocked only by missing date plumbing in pipeline.run_single_stock — full_scan_once takes no `date`. Not a provider refusal. | 2026-08-16 |
| option_chain_per_strike | freshness_only | none | none |  | equity_session | UW serves this endpoint for past dates (probed 2026-08-16, HTTP 200 with rows). Blocked only by missing date plumbing in pipeline.run_single_stock — full_scan_once takes no `date`. Not a provider refusal. | 2026-08-16 |
| option_contract_snapshots | freshness_only | none | none |  | equity_session | UW serves this endpoint for past dates (probed 2026-08-16, HTTP 200 with rows). Blocked only by missing date plumbing in pipeline.run_single_stock — full_scan_once takes no `date`. Not a provider refusal. | 2026-08-16 |
...
| pcr_history | freshness_only | none | none |  | equity_session | UW serves this endpoint for past dates (probed 2026-08-16, HTTP 200 with rows). Blocked only by missing date plumbing in pipeline.run_single_stock — full_scan_once takes no `date`. Not a provider refusal. | 2026-08-16 |
| risk_reversal_skew_history | freshness_only | none | none |  | equity_session | UW serves this endpoint for past dates (probed 2026-08-16, HTTP 200 with rows). Blocked only by missing date plumbing in pipeline.run_single_stock — full_scan_once takes no `date`. Not a provider refusal. | 2026-08-16 |

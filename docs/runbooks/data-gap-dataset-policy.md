...
| spx_density_forecast | research_artifact | db | none |  | equity_session | Display-only v13 density cone shadow log. Prospective rows are forward-in-time only; historical fill is origin='reconstructed', which the cone's own nightly job self-heals over the last 10 sessions (worker/jobs/spx_density_forecast._reconstruct_pass) and scripts/backfill/spx_density_backfill.py seeds beyond that. Neither may relabel a prospective row, so this stays outside the generic healer. |  |

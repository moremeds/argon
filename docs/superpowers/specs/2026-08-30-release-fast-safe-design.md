...
The committed duration manifest is regenerated by executing the complete integration
suite with `--store-durations`. Every baseline node ID must remain collected; the
fixture batching change adds one real-Postgres regression test.

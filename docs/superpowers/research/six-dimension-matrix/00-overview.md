...
> Updated 2026-05-15 per plan §"Known inconsistencies" — was positive skew z = vol-down / negative skew z = vol-up, now positive skew z = vol-up / negative skew z = vol-down.
...
| **Skew** | `skew_25d_zscore_180d < −1.0` (compressed: UW value less positive than 180d mean → puts relatively less rich) OR `skew_25d_5d_change < 0` (smirk relaxing) | `skew_25d_zscore_180d > +1.0` (extreme positive: puts richer than usual) OR `skew_25d_5d_change > +2σ` (accelerated steepening) | z-score in (−1, +1) and 5d change in (−2σ, +2σ) |

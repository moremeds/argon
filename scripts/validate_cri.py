...
print(
    f"Aligned: n={len(panel)}, range={panel.index.min().date()} -> {panel.index.max().date()}"
)
...
def cri_v1(row):
    return (
        score_vix(row["VIX"], row["VIX_5d_roc"])
...
panel["baseline_score"] = panel[
    "VIX"
]  # raw VIX as predictor (for AUC), threshold = p80
...
print(
    f"After dropping warmup + tail: n={len(labels_panel)}, "
...
print(
    f"OUT-OF-SAMPLE: n={len(oos)} ({oos.index.min().date()} -> {oos.index.max().date()})"
)
...
thr_v1 = find_threshold_for_alarm_rate(
    in_sample["CRI_v1"].values, baseline_alarm_rate_is
)
thr_v2 = find_threshold_for_alarm_rate(
    in_sample["CRI_v2"].values, baseline_alarm_rate_is
)
...
def evaluate(df, label_col, label_name):
    ...
    print(
        f"{'Predictor':<35}  {'AUC':>6}  {'Prec':>6}  {'Recall':>7}  {'F1':>6}  {'Alarm':>6}"
    )
    # Baseline (per-row threshold, no tuning needed)
    baseline_pred = baseline_threshold_per_row(df)
    base_metrics = {
        "precision": (
            baseline_pred[df[label_col] == 1].sum() / max(baseline_pred.sum(), 1)
        ),
        "recall": (
            baseline_pred[df[label_col] == 1].sum() / max(df[label_col].sum(), 1)
        ),
    ...
    base_metrics["f1"] = (
        ...
        * base_metrics["recall"]
        ...
        if base_metrics["precision"] + base_metrics["recall"]
        else float("nan")
    )
    base_auc = roc_auc(df[label_col], df["VIX"])
    print(
        f"{'baseline (VIX>=trailing p80)':<35}  {base_auc:>6.3f}  {base_metrics['precision']:>6.1%}  "
    ...
    print(
        f"{f'CRI v1 (thr={thr_v1:.1f})':<35}  {v1_auc:>6.3f}  {v1_m['precision']:>6.1%}  "
    ...
    print(
        f"{f'CRI v2 (thr={thr_v2:.1f})':<35}  {v2_auc:>6.3f}  {v2_m['precision']:>6.1%}  "
...
oos["c_vvix_v1"] = oos.apply(
    lambda r: score_vvix_v1(r["VVIX"], r["VVIX_VIX_ratio"]), axis=1
)
oos["c_vvix_v2"] = oos.apply(
    lambda r: score_vvix_v2(r["VVIX"], r["VVIX_VIX_ratio"], r["VVIX_5d_roc"]), axis=1
)
...
print(
    f"{'Event':<22}  {'Date':<12}  {'days_v1_first_alarm':>22}  {'days_v2_first_alarm':>22}  {'days_baseline':>16}"
)
for date, name in events:
    ...
    print(
        f"{name:<22}  {target_date.date()}  {str(v1_lead) + ' cal d':>22}  "

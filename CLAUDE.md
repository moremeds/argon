...
| Research ticker cohorts (non-watchlist) + their nightly capture | `storage/research_universe.py` + `worker/jobs/option_surface_research_capture.py` (19:10 ET, uw-0, gated `OPTION_SURFACE_RESEARCH_CAPTURE_ENABLED`, self-gates on an empty cohort) + migration `110` + seeding/backfill `scripts/research/option_surface_research_backfill.py`. Cohort `liquid_sector_balanced_v1` = 37 names, 10 sectors, marketcap ≥ $30B AND option OI ≥ 200k. Exists to counterweight the watchlist's AI/semi concentration — see `docs/research/2026-07-29-theta-harvester-loss-anatomy.md` |
...
| Scanner (detectors + ranking + discovery) | `src/uw_scan/scanner/` (pipeline, signals, ranking, discovery, gates, context) + `api/routers/scanner.py` + `web/app/scanner/[[...tab]]/page.tsx` (Flow / Discover / Theta sub-tabs, `components/scanner/ScannerPanel.tsx`) |

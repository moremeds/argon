...
/**
 * Radar, as the fundamentals desk's TRIAGE tab.
 *
 * The table itself moved in unchanged — this is a re-parenting, not a rewrite.
 * Radar answers "which names does Argon hold a compatible run for", which is
 * the question you ask BEFORE opening a desk, so it belongs under the desk
 * rather than beside it in the top-level nav.
 *
 * `/radar` still resolves: it redirects here. The old URL is in browser
 * histories and in this repo's own docs, and breaking it to tidy a route tree
 * would trade a real cost for a cosmetic gain.
 */
export default async function FundamentalsRadarPage({
  searchParams,
}: {

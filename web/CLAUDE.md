...
npm run test:e2e:technicals  # playwright --config playwright.technicals.config.ts
npm run gen:types    # regenerate lib/types.ts from FastAPI openapi.json
```

Extra Playwright configs beyond the default `playwright.config.ts`: `playwright.technicals.config.ts`, `playwright.config.canary.ts`, `playwright.config.canary-screenshot.ts`, `playwright.worktree.config.ts`.

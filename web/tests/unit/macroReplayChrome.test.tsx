...
describe("ReplayStatus — the answer, driven by the response", () => {
  it("says nothing when the page is live", () => {
    const { container } = render(
      <ReplayStatus verdict={{ kind: "not_replaying" }} publisher="rates snapshot" />,
    );
    expect(container.innerHTML).toBe("");
  });
  ...
  it("keeps unanswered, failed and withheld as three different sentences", () => {
    ...
    for (const verdict of [
      ...
      const view = render(
        <ReplayStatus verdict={verdict} publisher="rates snapshot" />,
      );
  ...
  it("never prescribes", () => {
    ...
    expect(text).not.toMatch(/\bbuy\b|\bsell\b|position size|predicted return/i);
  });
});
...
describe("the registry declares a clock for every tab", () => {
  it("names one of the three questions, never leaves it to be inferred", () => {
    ...
    // cannot do anything.
    expect(
      VALID_TABS.find((tab) => tab.slug === "notes")?.replayClock,
    ).toBe("none");
  });
});

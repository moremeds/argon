...
describe("TradeInsightsAiAnalysisPanel", () => {
  ...
  it("runs, polls, and renders the structured card grid", async () => {
    ...
    vi.mocked(api.tradeInsightsAiAnalysisStatus).mockResolvedValueOnce(succeededResponse());
    render(<TradeInsightsAiAnalysisPanel ticker="TSLA" />);

    fireEvent.click(screen.getByText("Run AI Analysis"));

    await waitFor(() => expect(api.tradeInsightsAiAnalysis).toHaveBeenCalledWith("TSLA", {}));
    expect(await screen.findByText("BUY setup")).toBeDefined();
    expect(screen.getByText("TSLA near gamma resistance with cheap vol and bullish flow")).toBeDefined();
    ...
    expect(screen.getByText("No event calendar data in deterministic payload.")).toBeDefined();
    expect(screen.getByText(/Generated analysis from local Codex/i)).toBeDefined();

...
def _snapshot_from_row(row: tuple[Any, ...]) -> PipelineBenchmarkSnapshotRow:
    return PipelineBenchmarkSnapshotRow(
        ...
        uw_latency_p95_ms=row[20],
        uw_http_429=row[21],
        uw_http_4xx=row[22],
        uw_http_5xx=row[23],
        requests_per_minute=row[24],
        scheduler_heartbeat_lag_seconds=row[25],
        uw_worker_online_count=row[26],
        uw_worker_expected_count=row[27],
        massive_worker_online_count=row[28],
        massive_worker_expected_count=row[29],
        ws_tick_age_seconds=row[30],
        record_health_ok=row[31],
        failing_record_tables=list(row[32] or []),
        details_jsonb=dict(row[33] or {}),
    )

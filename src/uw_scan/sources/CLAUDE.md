...
- `xenon_query.py` — Read-only **request/response** client for xenon's IB query API (distinct from the `xenon_ws.py` spot feed). `fetch_ib_option_iv` / `fetch_ib_option_quote` → `GET /options/greeks` for one contract's NBBO + IV (greeks are BS-computed downstream, never stored — IB theta is per-day). Default `XENON_QUERY_API_URL=http://127.0.0.1:8321` (the mini's authenticated localhost port; `:8421` was stale/dead) + **required** `XENON_QUERY_API_KEY` (`X-API-Key`; 401 without it). Each call spawns an IB snapshot subprocess (~2–5 s) + 1 IB line — never bulk-poll. Never-raise (returns `None`/empty on any error) so callers fall back to UW. See the root CLAUDE.md "Xenon read-only query API" section.
